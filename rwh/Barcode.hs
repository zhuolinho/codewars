...
import           Data.Array (Array(..), (!), bounds, elems, indices, ixmap
                           , listArray)
...
encodeDigits
  s@(first:rest) = outerGuard:lefties ++ centerGuard:righties ++ [outerGuard]
  where
...
parseRawPPM = parseWhileWith w2c (/= '\n')
  ==> \header
  -> skipSpaces ==>& assert (header == "P6") "invalid raw header" ==>& parseNat
  ==> \width -> skipSpaces ==>& parseNat
  ==> \height -> skipSpaces ==>& parseNat
  ==> \maxValue -> assert (maxValue == 255) "max value out of spec"
  ==>& parseByte
...
parseRGB :: Parse RGB
parseRGB = parseByte
  ==> \r -> parseByte ==> \g -> parseByte ==> \b -> identity (r, g, b)
...
useDigit old new digit = new
  `M.union` M.foldrWithKey (updateMap digit) M.empty old
...
firstDigit =
  snd . head . bestScores paritySRL . runLengths . map parityBit . take 6
  where
...
buildMap = M.mapKeys (realCheckDigit) . addFirstDigit . finalDigits
  where
...
findMatch =
  listToMaybe . filter (not . null) . map (solve . candidateDigits) . tails
